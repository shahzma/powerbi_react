...
//get id and reporturl and accesstoken from click
  ...
  const [ form_id, setForm_id ] = useState('702');
  ...
  function openModal(company_name, ques_name) {
    ...
    fetch(`http://127.0.0.1:8000/formID/`, {
      method: 'POST',
      body: uploadData
    }).then(data => data.json())
    .then( data => {
      console.log('data=',data)
      setForm_id(data.report_version_id)
      setQues_id(data.question_id)
      })
    .catch(error => {
      // setSignIn(false);
...
const postComment = async(comment, formId, instanceId, questionId)=>{
  ...
  console.log(formId, instanceId, questionId)
  if (comment?.length){
